    ...
    // Old spray-paint loader removed; replaced by new simple `#siteLoader` implementation.
        ...
        (function(){
            ...
            // If user already set preference, skip dialog
            const pref = localStorage.getItem('bgm_allowed');
            if(pref === 'true'){
                // start loader and attempt to play
                startSiteLoader();
                // attempt playback once after a short delay
            ...
            consentOn.addEventListener('click', ()=>{
                localStorage.setItem('bgm_allowed','true');
                // allow audio playback; browsers typically require user gesture which this click provides
                audio.play().catch(()=>{});
                startSiteLoader();
                hideConsent();
            });
            ...
            consentOff.addEventListener('click', ()=>{
                localStorage.setItem('bgm_allowed','false');
                // ensure audio is paused and muted
